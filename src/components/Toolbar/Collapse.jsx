...
class ToolbarCollapse extends React.Component {
  ...
  render() {
    ...
    if (
      (direction === 'column' &&
        ['left-bottom', 'center-bottom', 'right-bottom'].indexOf(position) >
          -1) ||
      (direction === 'row' &&
        ['right-top', 'right-center', 'right-bottom'].indexOf(position) > -1)
    ) {
      elements.reverse();
    }

    return <Container className={classes}>{elements}</Container>;
  }
}
